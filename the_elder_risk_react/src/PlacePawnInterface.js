...
class PlacePawnInterface extends React.Component{ 
    ...
    state = {
        pawn: 1,
        messageError:null,
        changeClass:"hide-phase-popup",
        styles:{display:"block"}
    }

    findMyPawn = () =>{
        for(let i = 0; i < this.props.game.playerList.length; i++){
            if(this.props.userName == this.props.game.playerList[i].name){
                return this.props.game.playerList[i].pawnsToPlace;
            }
        }
    }
    ...
    render (){
        return(
            <div  className={"container-addpawns-phase " }>
                ...
                {this.findMyPawn() <= 0 ? <p>Wait players placed their pawns</p> : <p>You have <strong className="pawns-number">{this.findMyPawn()}</strong> pawns to place</p>}
                {this.findMyPawn() <= 0 ? "" : <button id="add-pawns-button-popup" onClick={() => this.displayPhasePopUp()}>Add your pawns</button>}
        </div>
            </div>
            )      
    }
